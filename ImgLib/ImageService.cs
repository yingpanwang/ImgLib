...
public sealed partial class ImageService
{
    ...
    public static void GenerateWithContext(ImageGenContext context)
    {
        ...
        DrawMainContent(original, canvas, destRect, context.Options);

        DrawWatermarkText(original, canvas, destRect, context.Options);
        ...
        static void DrawMainContent(SKBitmap original, SKCanvas canvas, in SKRect destRect, in ImageGenerateOption option)
        {
            float blurSigma = option.BlurSigma;
            float width = original.Width;
            float height = original.Height;
            float shadowOffsetX = option.ShadowOffsetX;
            float shadowOffsetY = option.ShadowOffsetY;
            float shadowSigma = option.ShadowSigma;
            float cornerRadius = option.CornerRadius;
        ...
        static void DrawWatermarkText(SKBitmap original, SKCanvas canvas, in SKRect destRect, in ImageGenerateOption option)
        {
            #region 水印文本
            ...
            float height = original.Height;
            float newHeight = original.Height * option.Scale;
            float y = (height - newHeight) / 2;
