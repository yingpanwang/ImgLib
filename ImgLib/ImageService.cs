...
{
    record struct BackgroundConfig()
    {
    ...
    {
        GenerateWithOptions(inputStream, outputStream);
        return;
        ...
        //// 阴影
        //float shadowOffsetRatio = 0.8f; // 20% 大小的偏移
        //float dx = width * shadowOffsetRatio;
        //float dy = height * shadowOffsetRatio;

        //var shadow = SKImageFilter.CreateDropShadow(dx, dy, 15, 15, SKColors.Black.WithAlpha(128));
        ...
        //    IsAntialias = true
        //};
        //canvas.DrawBitmap(original, destRect, shadowPaint);

        float cornerRadius = 45; // 圆角半径

        canvas.ClipRoundRect(new SKRoundRect(destRect, cornerRadius, cornerRadius), SKClipOperation.Intersect, true);
    ...
    {
        public SKBitmap Origin { get; init; }
